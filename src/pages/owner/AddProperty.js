...
export default function AddProperty() {
    let {register, handleSubmit, formState: {errors}, getValues, setValue, reset} = useForm();

    function getownername() {
        axios.get("http://localhost:4000/owner/getowneremail").then((response) => {
            setValue("owner_email", response.data);
        }).catch((error) => {
            console.log(error);
        })
    ...
    function onSubmit(data) {
        ...
        axios.post("http://localhost:4000/owner/owner-add-property", formData).then((response) => {
            if (response.data === "success") {
                resetdetails();
                Swal.fire(
                    'Request Sent!',
                    'Your property request has been sent to admin',
                    'info'
                )
                ...
                Swal.fire(
                    'Error',
                    'Property cannot be added. Please try again after sometime!',
                    'error'
                )
            }
        }).catch((error) => {
    ...
    return (
        <>
            <div className="text-center mb-3">
                <h3 className={"style-font me-5"}>ADD PROPERTY</h3>
            </div>
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    <div className={"row col-md-12"} style={{paddingRight: "-10px"}}>
                        ...
                        <div className={"mt-5 row col-md-7"}>
                            ...
                            <div className="form-group col-md-6 ">
                                <select
                                    className={"fc-custom"} {...register('category_name', {required: "You must specify category name"})}>
                    ...
                    <div className={"col-md-4 offset-md-6"}>
                        <button className={"btn sent-butnn mr-3"}>Add property</button>
                        <Button variant={"contained"} className="btn danger"
